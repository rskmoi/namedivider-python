...
class NameDivider:
    def __init__(
        self, path_csv: str = f"{CURRENT_DIR}/assets/kanji.csv", separator: str = " "
    ):
        """
        ...
        for _kanji, _order, _length in zip(kanjis, orders, lengths):
            self.kanji_dict[_kanji] = KanjiStatistics(
    ...
    def _calc_order_score(
        self, piece_of_divided_name: str, full_name_length: int, start_index: int = 0
    ) -> float:
        """
        ...
        for idx_in_piece_of_divided_name, _kanji in enumerate(piece_of_divided_name):
            ...
            masked_order = (
                self.kanji_dict.get(_kanji, self.default_kanji).order_counts * mask
            )
    ...
    def _calc_length_score(
        ...
        for i, _kanji in enumerate(piece_of_divided_name):
            ...
            current_length_status_idx = self._calc_current_length_status(
                piece_of_divided_name, is_family
            )
            masked_length_scores = (
                self.kanji_dict.get(_kanji, self.default_kanji).length_counts * mask
            )
            if np.sum(masked_length_scores) == 0:
                continue
            scores += masked_length_scores[current_length_status_idx] / np.sum(
                masked_length_scores
            )
        return scores
