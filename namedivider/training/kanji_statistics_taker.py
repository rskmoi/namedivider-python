...
class KanjiStatisticsTaker:
    ...
    def __init__(
        self, mode: KanjiStatisticsMode = KanjiStatisticsMode.ONLY_FREQUENT_KANJI
    ):
    ...
    def to_csv(self, dst: str | Path) -> None:
        stats: list[tuple[str, KanjiStatistics]] = sorted(
            self.statistics.items(), key=lambda x: x[0]
        )
        items = []
