...
def get_feature_and_target(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    X = df[
        [
            "rank",
            "fullname_length",
...
def train(src_train: str, src_valid: str, dst: str) -> None:
    ...
    params = {"objective": "binary", "metric": "auc", "n_estimators": 10000}
    gbm = lgb.train(
        params, lgb_train, valid_sets=[lgb_valid], early_stopping_rounds=200
    )
    gbm.save_model(dst)
