...
@st.experimental_singleton  # type: ignore[attr-defined,misc]
def get_basic_name_divider() -> BasicNameDivider:
    basic_divider = BasicNameDivider()
    return basic_divider


@st.experimental_singleton  # type: ignore[attr-defined,misc]
def get_gbdt_name_divider() -> GBDTNameDivider:
...
def main() -> None:
    col1, *_ = st.columns(5)
    with col1:
        option = st.selectbox("Language", ("Japanese", "English"))
    ...
    if option == "Japanese":
        ...
        info = (
            "このページはNameDividerの使用感をお伝えするためのデモページです。\n "
        ...
        st.write("https://github.com/rskmoi/namedivider-python")
        st.write(
            "Entering names in the text area on the left side will output the divided names on the right side."
        )
        ...
        too_many_error = (
            "The number of names that can be divided at one time is limited to 500"
        )
        info = (
            "This page is a demo page to show how NameDivider works.\n "
            "1. The author assumes no responsibility for "
            "any disadvantages that may result from the use of this page. \n "
    ...
    mode = st.radio(mode_select, (mode_basic, mode_gbdt))

    input_area, output_area = st.columns(2)
    undivided_names = input_area.text_area(
        input_label, height=500, placeholder="菅義偉\n岸田文雄\n鳩山由紀夫"
    )
    divided_names = []
